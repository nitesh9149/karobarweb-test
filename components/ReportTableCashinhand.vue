  ...
  <div>
    ...
    >
      <template slot-scope="props">
        ...
        <b-table-column class="pad_left" label="Cash In" field="amount">
          <span v-if="props.row.money_in_value"
            >Rs. {{ props.row.money_in_value }}</span
          >
          <span v-else>-</span>
        </b-table-column>
        <b-table-column class="pad_left" label="Cash Out" field="mode">
  ...
  watch: {
    async dateSelect1(date1) {
      ...
      let response = await axios.get(
        "/transactions/reports/?type=cash-in-hand&business=" +
      ...
      this.$emit("excelData", this.clients, allAmt, "cash");
      this.loading = false;
    },
  },
  methods: {
