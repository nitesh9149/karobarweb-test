    ...
    >
      <template slot-scope="props">
        ...
        <b-table-column
          ...
          field="transaction"
        >
          <span v-if="props.row.payment_mode_value">
            {{ props.row.payment_mode_value }}
          </span>
          <span v-else>-</span>
        </b-table-column>
        <b-table-column class="pad_left" label="Name" field="name">
          <span v-if="props.row.party_value">
            {{ props.row.party_value }}
          </span>
          <span v-else>-</span>
        </b-table-column>
        <b-table-column class="pad_left" label="Money In" field="balance">
          <span v-if="props.row.money_in_value"
            >Rs. {{ conversion(props.row.money_in_value) }}</span
          >
          <span v-else>-</span>
        </b-table-column>
        <b-table-column class="pad_left" label="Money Out" field="cost">
  ...
  async mounted() {
    ...
    for (let i = 0; i < response.data.results.length; i++) {
      ...
      balanceList.push({
        ...
        amount:
          response.data.results[i].type == "add-money"
            ? Math.abs(response.data.results[i].amount)
            : response.data.results[i].type == "reduce-money"
            ? -Math.abs(response.data.results[i].amount)
            : response.data.results[i].amount,
        ...
        payment_mode: response.data.results[i].payment_mode,
    ...
    let net_amount = Number(stat.data.bank_transactions);
        ...
        if (
          ...
          for (let i = 0; i < response.data.results.length; i++) {
            ...
            balanceList.unshift({
              ...
              amount_value:
                response.data.results[i].type == "add-money"
                  ? Math.abs(response.data.results[i].received)
                  : response.data.results[i].type == "reduce-money"
                  ? -Math.abs(response.data.results[i].received)
                  : response.data.results[i].received,
              party: response.data.results[i].party,
              party_value:
                response.data.results[i].party_value == "" ||
                response.data.results[i].party_value == null
