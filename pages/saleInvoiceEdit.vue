      ...
      <div class="level">
        <div class="level-left">
          <div class="level-item">
            <ul>
              <li v-if="globalHelper('editSale')">Edit Sales Invoice</li>
              <li v-else>Sales Invoice</li>
            </ul>
          </div>
        </div>
...
export default {
  ...
  async mounted() {
    ...
    try {
      ...
      this.sale.id = res.data.sale.id;
      if (res.data.sale.on_cash == true) {
        this.name = "Cash Sale";
        this.sale.on_cash = true;
        this.sale.party_value = null;
      } else {
        this.name = res.data.sale.party_name;
        this.sale.party = res.data.sale.party;
      }
      if (res.data.sale.image_url_1) {
        this.invoice_images[0] = res.data.sale.image_url_1;
      }
      if (res.data.sale.image_url_2) {
        this.invoice_images[1] = res.data.sale.image_url_2;
      }
      if (res.data.sale.image_url_3) {
        this.invoice_images[2] = res.data.sale.image_url_3;
      }
      this.sale.number = res.data.sale.number;
      let date2 = new Date(res.data.sale.date);
      let nepDate2 = new NepaliDate(date2).format("YYYY-MM-DD");
      this.sale.date = res.data.sale.date;
      this.date = nepDate2;
      if (res.data.sale.due) {
        this.sale.due = res.data.sale.due;
        this.displayDue = true;
        let due = new Date(res.data.sale.due);
        let nepDate = new NepaliDate(due).format("YYYY-MM-DD");
        this.dueDate = nepDate;
      }
      this.sale.note = res.data.sale.note;
      if (res.data.sale.vat_enabled) {
        this.addVat();
      }
      this.totalDataDis = res.data.sale.total;
      this.receiveData = res.data.sale.received;
      this.currentUnit.text =
        res.data.sale.payment_mode.charAt(0).toUpperCase() +
        res.data.sale.payment_mode.slice(1);
      ...
      if (res.data.sale.discount_percent > 0) {
        this.discountPercent = res.data.sale.discount_percent;
        this.discountPer = res.data.sale.discount_percent;
        this.d_value = res.data.sale.discount_percent;
        this.addDiscount();
        this.updateDiscountAmount(res.data.sale.discount_percent);
      }
    } catch (err) {}
    let list = [];
    try {
      ...
      if (this.sale.party_name == "") {
        ...
        if (this.totalSum >= 0) {
          ...
          try {
            ...
            this.$buefy.snackbar.open({
              message: "Sale Invoice has been updated",
              type: "is-primary",
              position: "is-bottom",
              indefinite: false,
            });
    ...
    async trashConfirm2() {
      ...
      try {
        const response = await axios.delete(
          "/transactions/sales/" + localStorage.getItem("sale") + "/",
          {
            headers: {
              Authorization: localStorage.getItem("auth._token.local"),
            },
        ...
        this.$buefy.snackbar.open({
          message: "Sale Invoice has been deleted",
          type: "is-primary",
          position: "is-bottom",
          indefinite: false,
        });
  ...
  head() {
    return {
      title: "Edit Sales Invoice - Karobar",
    };
  },
};
</script>
