      ...
      <div class="level">
        <div class="level-left">
          <div class="level-item">
            <ul>
              <li v-if="globalHelper('editSale')">Convert to Sales Invoice</li>
...
export default {
  ...
  async mounted() {
    ...
    try {
      ...
      this.sale.id = res.data.estimate.id;
      if (res.data.estimate.on_cash == true) {
        this.name = "Cash Sale";
        this.sale.on_cash = true;
        this.sale.party_value = null;
      } else {
        this.name = res.data.estimate.party_name;
        this.sale.party = res.data.estimate.party;
      }
      this.sale.number = res.data.estimate.number;
      let date2 = new Date(res.data.estimate.date);
      let nepDate2 = new NepaliDate(date2).format("YYYY-MM-DD");
      this.sale.date = res.data.estimate.date;
      this.date = nepDate2;
      if (res.data.estimate.due) {
        this.sale.due = res.data.estimate.due;
        this.displayDue = true;
        let due = new Date(res.data.estimate.due);
        let nepDate = new NepaliDate(due).format("YYYY-MM-DD");
        this.dueDate = nepDate;
      }
      this.sale.note = res.data.estimate.note;
      if (res.data.estimate.vat_enabled) {
        this.addVat();
      }
      this.totalDataDis = res.data.estimate.total;
      this.receiveData = res.data.estimate.received;
      this.currentUnit.text =
        res.data.estimate.payment_mode.charAt(0).toUpperCase() +
        res.data.estimate.payment_mode.slice(1);
      ...
      if (res.data.estimate.discount_percent > 0) {
        this.addDiscount();
        this.updateDiscountAmount(res.data.estimate.discount_percent);
        this.d_value = res.data.estimate.discount_percent;
      }
    } catch (err) {}
    let list = [];
    try {
    ...
    async saveInvoice() {
      ...
      if (this.sale.party_name == "") {
        ...
        try {
          ...
          this.$buefy.snackbar.open({
            message: "Estimate has been converted to Sale Invoice",
            type: "is-primary",
            position: "is-bottom",
            indefinite: false,
          });
    ...
    async trashConfirm2() {
      ...
      try {
        const response = await axios.delete(
          "/transactions/estimates/" + localStorage.getItem("estimate") + "/",
          {
            headers: {
              Authorization: localStorage.getItem("auth._token.local"),
            },
        ...
        this.$buefy.snackbar.open({
          message: "Estimate has been deleted",
          type: "is-primary",
          position: "is-bottom",
          indefinite: false,
        });
