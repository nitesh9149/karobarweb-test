      ...
      <div class="level">
        <div class="level-left">
          <div class="level-item">
            <ul>
              <li v-if="globalHelper('editSale')">Edit Sales Return</li>
              <li v-else>Sales Return</li>
            </ul>
          </div>
        </div>
        ...
        <div class="box">
          ...
          <div class="columns">
            ...
            <div class="column is-5 is-offset-1">
              <div class="buttons">
                <b-button
                  v-if="globalHelper('editSale')"
                  @click="saveInvoice"
                  type="is-success"
                  >SAVE SALE RETURN</b-button
                >
              </div>
            </div>
          </div>
...
export default {
  ...
  async mounted() {
    ...
    try {
      ...
      this.sale.id = res.data.sale_return.id;

      if (res.data.sale_return.on_cash == true) {
        this.name = "Cash Sale Return";
        this.sale.on_cash = true;
        this.sale.party_value = null;
      } else {
        this.name = res.data.sale_return.party_name;
        this.sale.party = res.data.sale_return.party;
      }
      if (res.data.sale_return.image_url_1) {
        this.invoice_images[0] = res.data.sale_return.image_url_1;
      }
      if (res.data.sale_return.image_url_2) {
        this.invoice_images[1] = res.data.sale_return.image_url_2;
      }
      if (res.data.sale_return.image_url_3) {
        this.invoice_images[2] = res.data.sale_return.image_url_3;
      }
      this.sale.number = res.data.sale_return.number;
      let date2 = new Date(res.data.sale_return.date);
      let nepDate2 = new NepaliDate(date2).format("YYYY-MM-DD");
      this.date = nepDate2;
      if (res.data.sale_return.due) {
        this.sale.due = res.data.sale_return.due;
        this.displayDue = true;
        let due = new Date(res.data.sale_return.due);
        let nepDate = new NepaliDate(due).format("YYYY-MM-DD");
        this.dueDate = nepDate;
      }
      this.sale.note = res.data.sale_return.note;
      if (res.data.sale_return.vat_enabled) {
        this.addVat();
      }
      this.totalDataDis = res.data.sale_return.total;
      this.receiveData = res.data.sale_return.paid;
      this.currentUnit.text =
        res.data.sale_return.payment_mode.charAt(0).toUpperCase() +
        res.data.sale_return.payment_mode.slice(1);
      ...
      if (res.data.sale_return.discount_percent > 0) {
        this.discountPercent = res.data.sale_return.discount_percent;
        this.discountPer = res.data.sale_return.discount_percent;
        this.d_value = res.data.sale_return.discount_percent;
        this.addDiscount();
        this.updateDiscountAmount(res.data.sale_return.discount_percent);
      }
    } catch (err) {}
    let list = [];
    try {
      ...
      list.unshift({
        name: "Cash Sale Return",
        phone_number: "Select this option if the sale return is in cash",
        value: "cash sale",
      });
      this.data = list;
    } catch (err) {}
  ...
  computed: {
    titleStack() {
      return ["Edit Sales Return Invoice"];
    },
    titleSub() {
      return ["Items"];
    },
    ...
    receiveDisplay: function () {
      ...
      if (this.selected != null) {
        if (
          ...
          this.name.trim().toLowerCase() == "cash sale return"
        ) {
          this.sale.on_cash = true;
        ...
        if (this.totalDataDis > 0) {
          if (this.name.trim().toLowerCase() == "cash sale return") {
            this.receiveField = true;
          } else {
            this.receiveField = false;
          }
  ...
  methods: {
    ...
    async onSelectFile() {
      ...
      if (files && files[0]) {
        try {
          ...
          formData.append("type", "sale-return");
    ...
    saveData() {
      ...
      if (!this.receiveData) {
        ...
        this.sale.paid = this.receiveData;
      }
      let list = [];
      ...
      if (this.sale.party_name == "") {
        ...
        if (this.totalSum >= 0) {
          ...
          try {
            ...
            this.$buefy.snackbar.open({
              message: "Sale Return has been updated",
              type: "is-primary",
              position: "is-bottom",
              indefinite: false,
            });
  ...
  head() {
    return {
      title: "Edit Sales Return Invoice - Karobar",
    };
  },
};
</script>
