      ...
      <div class="level">
        <div class="level-left">
          <div class="level-item">
            <ul>
              <li v-if="globalHelper('editPurchase')">Edit Purchase Return</li>
              <li v-else>Purchase Return</li>
            </ul>
          </div>
        </div>
          ...
          <div class="level-item">
            <div class="buttons is-right">
              <b-button @click="generateReport" type="is-primary">
                ...
                <span class="button-title">Generate Invoice</span>
              </b-button>
            </div>
          </div>
        </div>
          ...
          <div class="columns">
            <div class="column is-6">
              <b-button
                ...
                @click="trashModal2"
                class="button-trash is-danger-outline"
              >
...
import TodoTableEdit from "@/components/TodoTableEdit";
...
export default {
  ...
  async mounted() {
    ...
    try {
      ...
      this.sale.id = res.data.purchase_return.id;
      if (res.data.purchase_return.on_cash == true) {
        this.name = "Cash Purchase Return";
        this.sale.on_cash = true;
        this.sale.party_value = null;
      } else {
        this.name = res.data.purchase_return.party_name;
        this.sale.party = res.data.purchase_return.party;
      }
      if (res.data.purchase_return.image_url_1) {
        this.invoice_images[0] = res.data.purchase_return.image_url_1;
      }
      if (res.data.purchase_return.image_url_2) {
        this.invoice_images[1] = res.data.purchase_return.image_url_2;
      }
      if (res.data.purchase_return.image_url_3) {
        this.invoice_images[2] = res.data.purchase_return.image_url_3;
      }
      this.sale.number = res.data.purchase_return.number;
      let date2 = new Date(res.data.purchase_return.date);
      let nepDate2 = new NepaliDate(date2).format("YYYY-MM-DD");
      this.date = nepDate2;
      if (res.data.purchase_return.due) {
        this.sale.due = res.data.purchase_return.due;
        this.displayDue = true;
        let due = new Date(res.data.purchase_return.due);
        let nepDate = new NepaliDate(due).format("YYYY-MM-DD");
        this.dueDate = nepDate;
      }
      this.sale.note = res.data.purchase_return.note;
      if (res.data.purchase_return.vat_enabled) {
        this.addVat();
      }
      this.totalDataDis = res.data.purchase_return.total;
      this.receiveData = res.data.purchase_return.received;
      this.currentUnit.text =
        res.data.purchase_return.payment_mode.charAt(0).toUpperCase() +
        res.data.purchase_return.payment_mode.slice(1);
      if (res.data.billing_items.length) {
        this.billItem = res.data.billing_items;
      }
      if (res.data.additional_charges.length) {
        this.isShowLabel = false;
        let addCharges = [];
        res.data.additional_charges.forEach((e) => {
          addCharges.push({
            id: e.id,
            chargeId: e.id,
            name: e.title,
        ...
        this.charges = addCharges.reverse();
      }
      if (res.data.purchase_return.discount_percent > 0) {
        this.discountPercent = res.data.purchase_return.discount_percent;
        this.discountPer = res.data.purchase_return.discount_percent;
        this.d_value = res.data.purchase_return.discount_percent;
        this.addDiscount();
        this.updateDiscountAmount(res.data.purchase_return.discount_percent);
      }
    } catch (err) {}
    let list = [];
    try {
      ...
      response.data.results.forEach((element) => {
        list.push({
          ...
          amount:
            element.amount < 0
              ? "Rs" + " " + this.conversion(Math.abs(-element.amount))
              : element.amount > 0
              ? "Rs" + " " + this.conversion(Math.abs(element.amount))
      ...
      list.unshift({
        name: "Cash Purchase Return",
        phone_number: "Select this option if the purchase return is in cash",
        value: "cash sale",
      });
      this.data = list;
    } catch (err) {}
    ...
    receiveDisplay: function () {
      ...
      if (this.selected != null) {
        if (
          ...
          this.name.trim().toLowerCase() == "cash purchase return"
        ) {
          this.sale.on_cash = true;
        ...
        if (this.totalDataDis > 0) {
          if (this.name.trim().toLowerCase() == "cash purchase return") {
            this.receiveField = true;
          } else {
            this.receiveField = false;
          }
  ...
  methods: {
    ...
    inputPeriod(e) {
      if (e > 0) {
        ...
        afterToday.setDate(today.getDate() + Math.abs(e));
    ...
    trashConfirm(post) {
      ...
      this.data.unshift({
        ...
        amount:
          post.amount < 0
            ? this.conversion(Math.abs(-post.amount))
            : post.amount > 0
            ? this.conversion(Math.abs(post.amount))
    ...
    async onSelectFile() {
      ...
      if (files && files[0]) {
        try {
          ...
          formData.append("type", "purchase-return");
      ...
      if (this.sale.party_name == "") {
        ...
        if (this.totalSum >= 0) {
          ...
          try {
            ...
            this.$buefy.snackbar.open({
              message: "Purchase Return has been updated",
              type: "is-primary",
              position: "is-bottom",
              indefinite: false,
            });
    ...
    async trashConfirm2() {
      ...
      try {
        ...
        this.$buefy.snackbar.open({
          message: "Purchase Return has been deleted",
          type: "is-primary",
          position: "is-bottom",
          indefinite: false,
        });
  ...
  head() {
    return {
      title: "Edit Purchase Return - Karobar",
    };
  },
};
</script>
...
.sale-invoice-o {
  ...
  .sale-bottom {
    ...
    .button-trash.is-danger-outline {
      border-color: #e54f6d;
      box-shadow: none;
      color: #e54f6d;
